{
  ...
  services.jellyfin = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.rclone}/bin/rclone mount --config ${config.age.secrets.rclone-conf.path} --vfs-cache-mode full --allow-other --vfs-cache-max-size 500M --no-modtime --umask 022 NDCRYPT: /var/lib/jellyfin/media";
