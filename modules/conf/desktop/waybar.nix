{
  ...
  cfg = config.conf.desktop.waybar;
  ...
  styleCSS = with config.scheme.withHashtag;
  ...
  ''}/bin/waybar-${name}";
in {
  options.conf.desktop.waybar.enable = lib.mkEnableOption "Waybar";

  config.home-manager.users.${config.conf.users.user}.programs.waybar = lib.mkIf cfg.enable {
    enable = true;
    style = styleCSS;
    systemd.enable = true;
    settings = {
