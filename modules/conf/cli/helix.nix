{
  ...
  cfg = config.conf.cli.helix;
  alejandra = inputs.alejandra.defaultPackage.${pkgs.system};
  helixUnstable = inputs.helix.outputs.packages.${pkgs.system}.helix;
in {
  options.conf.cli.helix.enable = lib.mkEnableOption "Helix";

  config.home-manager.users.${config.conf.users.user}.programs.helix = lib.mkIf cfg.enable {
    enable = true;
    ...
    languages = [
      ...
      }
      {
        ...
        config.nil.formatting.command = ["${alejandra}/bin/alejandra" "-q"];
        auto-format = true;
      }
    ];
