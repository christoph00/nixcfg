...
in {
  ...
  config = mkIf cfg.enable {
    ...
    in {
      enable = true;
      openFirewall = true;
      package = package.overrideAttrs (o: { doInstallCheck = false; });
      ...
      config = {
        ...
        recorder = {
          ...
          exclude = {
            ...
            entity_globs = [ "sensor.weather_*" "sensor.date_*" ];
            entities =
              [ "sun.sun" "sensor.last_boot" "sensor.date" "sensor.time" ];
            event_types = [ "call_service" ];
          };
        };
        ffmpeg = { ffmpeg_bin = "${pkgs.ffmpeg}/bin/ffmpeg"; };
        wake_on_lan = { };
        utility_meter = { };
        zha = {
          enable_quirks = true;
