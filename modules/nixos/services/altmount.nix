            ...
            type = lib.types.submodule {
              options = {
                path = lib.mkOption {
                  type = lib.types.str;
                  default = "";
                  description = "RClone base path (defaults to config directory).";
                };

                password = lib.mkOption {
                  type = lib.types.str;
                  default = "";
                  description = "Encryption password (optional).";
                };

                salt = lib.mkOption {
                  type = lib.types.str;
                  default = "";
                  description = "Encryption salt (optional).";
                };

                rc_enabled = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
                  description = "Enable RC server connection.";
                };

                rc_url = lib.mkOption {
                  type = lib.types.str;
                  default = "";
                  description = "External RC server URL.";
                };

                rc_port = lib.mkOption {
                  type = lib.types.port;
                  default = 5573;
                  description = "Port for internal RC server.";
                };

                rc_user = lib.mkOption {
                  type = lib.types.str;
                  default = "admin";
                  description = "RC authentication username.";
                };

                rc_pass = lib.mkOption {
                  type = lib.types.str;
                  default = "admin";
                  description = "RC authentication password.";
                };

                rc_options = lib.mkOption {
                  type = lib.types.attrs;
                  default = {};
                  description = "Additional RC server options.";
                };

                mount_enabled = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
                  description = "Enable RClone mount.";
                };
                ...
                allow_other = lib.mkOption {
                  type = lib.types.bool;
                  default = true;
                  description = "Allow other users to access the mount.";
                };

                allow_non_empty = lib.mkOption {
                  type = lib.types.bool;
                  default = true;
                  description = "Allow mounting over non-empty directories.";
                };
                ...
                timeout = lib.mkOption {
                  type = lib.types.str;
                  default = "10m";
                  description = "I/O timeout for mount operations.";
                };
                ...
                log_level = lib.mkOption {
                  type = lib.types.str;
                  default = "INFO";
                  description = "Log level for rclone operations.";
                };
                ...
                buffer_size = lib.mkOption {
                  type = lib.types.str;
                  default = "32M";
                  description = "Buffer size for file operations.";
                };

                attr_timeout = lib.mkOption {
                  type = lib.types.str;
                  default = "1s";
                  description = "Attribute timeout for FUSE.";
                };
                ...
                cache_dir = lib.mkOption {
                  ...
                  description = "VFS cache directory.";
                };

                vfs_cache_mode = lib.mkOption {
                  type = lib.types.str;
                  default = "full";
                  description = "VFS cache mode: off|minimal|writes|full.";
                };

                vfs_cache_max_size = lib.mkOption {
                  type = lib.types.str;
                  default = "50G";
                  description = "Maximum cache size.";
                };

                vfs_cache_max_age = lib.mkOption {
                  type = lib.types.str;
                  default = "504h";
                  description = "Maximum cache age.";
                };

                read_chunk_size = lib.mkOption {
                  type = lib.types.str;
                  default = "32M";
                  description = "VFS read chunk size.";
                };

                read_chunk_size_limit = lib.mkOption {
                  type = lib.types.str;
                  default = "2G";
                  description = "Read chunk size limit.";
                };

                vfs_read_ahead = lib.mkOption {
                  type = lib.types.str;
                  default = "128M";
                  description = "VFS read-ahead size.";
                };

                dir_cache_time = lib.mkOption {
                  type = lib.types.str;
                  default = "10m";
                  description = "Directory cache time.";
                };

                vfs_cache_min_free_space = lib.mkOption {
                  type = lib.types.str;
                  default = "1G";
                  description = "Minimum free space for VFS cache.";
                };

                vfs_disk_space_total = lib.mkOption {
                  type = lib.types.str;
                  default = "1G";
                  description = "Total disk space for VFS operations.";
                };

                vfs_read_chunk_streams = lib.mkOption {
                  type = lib.types.int;
                  default = 4;
                  description = "Number of read chunk streams.";
                };

                no_mod_time = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
                  description = "Don't read/write modification time.";
                };

                no_checksum = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
                  description = "Don't verify checksums on upload.";
                };

                async_read = lib.mkOption {
                  type = lib.types.bool;
                  default = true;
                  description = "Enable async read operations.";
                };

                vfs_fast_fingerprint = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
                  description = "Use fast fingerprinting for VFS.";
                };

                use_mmap = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
                  description = "Use memory-mapped files for reading.";
                };
              };
            };
            default = {};
   ...
   age.secrets.altmount-cfg = mkSecret {
    ...
    systemd.services.altmount = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${lib.getExe cfg.package} -config /run/altmount/config.yaml";
      };
    };
