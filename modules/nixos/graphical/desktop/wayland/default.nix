...
let cfg = config.internal.graphical.desktop.wayland;
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services = mkIf cfg.uwsm {
      waybar = mkIf cfg.waybar {
        ...
        script =
          "unset __NIXOS_SET_ENVIRONMENT_DONE && . /run/current-system/etc/profile && ${pkgs.waybar}/bin/waybar";
      ...
      sfwbar = mkIf cfg.sfwbar {
        description = "sfwbar";
        script =
          "unset __NIXOS_SET_ENVIRONMENT_DONE && . /run/current-system/etc/profile && ${pkgs.sfwbar}/bin/sfwbar";
        wantedBy = [ "graphical-session.target" ];
        after = [ "graphical-session.target" ];
        serviceConfig.Slice = "app-graphical.slice";
      ...
      ironbar = mkIf cfg.ironbar {
        description = "ironbar";
        script =
          "unset __NIXOS_SET_ENVIRONMENT_DONE && . /run/current-system/etc/profile && ${pkgs.ironbar}/bin/ironbar";
        wantedBy = [ "graphical-session.target" ];
        after = [ "graphical-session.target" ];
        serviceConfig.Slice = "app-graphical.slice";
      };
