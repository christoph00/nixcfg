...
let
  ...
  mkWgPeer =
    ...
    }:
    {
      ...
      endpoint =
        if publicIP != null && publicIP != "dynamic" then
          "${publicIP}:${toString port}"
        else
          "${name}:${toString port}";
    };
  ...
  mkAutoPeer =
    ...
    let
      hostConfig =
        flake.nixosConfigurations.${name} or (throw "Host ${name} not found in flake.nixosConfigurations");
      ...
      resolvedIP =
        if ip != null then
          ...
          wgConfig.ip
            or (throw "No IP configured for ${name}. Make sure ${name}.config.network.wireguard.ip is set.");
      resolvedPubkey =
        if pubkey != null then
          pubkey
        else
          wgConfig.publicKey
            or (throw "No public key configured for ${name}. Make sure ${name}.config.network.wireguard.publicKey is set.");
  ...
  wireguardHosts = lib.filterAttrs (
    name: hostConfig: hostConfig.config.network.wireguard.enable or false
  ) flake.nixosConfigurations;

  autoPeers = lib.mapAttrs (
    name: hostConfig:
    mkAutoPeer name {
    }
  ) wireguardHosts;
  ...
  currentHost =
    autoPeers.${hostname} or manualPeers.${hostname}
      or (throw "Host ${hostname} not found in wgnet configuration");
